  ...
  paths: {
    ...
    '/tasks/{id}': {
      get: {
        ...
        responses: {
          '200': { description: 'Dados da tarefa' },
    ...
    '/tasks/{id}/status': {
      patch: {
        tags: ['Tasks'],
        summary: 'Atualizar status da tarefa (Kanban)',
        security: [{ bearerAuth: [] }],
    ...
    '/files/{id}/download': {
      get: {
        ...
        security: [{ bearerAuth: [] }],
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'integer' } }],
        responses: { '200': { description: 'Arquivo para download' } }
      }
    },
    '/tags': {
      get: {
          ...
          content: {
            'application/json': {
              schema: {
                type: 'object',
                required: ['name'],
                properties: { name: { type: 'string', example: 'urgente' } }
              }
            }
          }
        },
        responses: { '201': { description: 'Tag criada' }, '400': { description: 'Tag já existe' } }
      }
    },
    '/tags/{id}': {
      delete: {
        ...
        security: [{ bearerAuth: [] }],
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'integer' } }],
        responses: { '200': { description: 'Tag excluída' } }
      }
    },
    '/tags/tasks/{taskId}/tags/{tagId}': {
      post: {
        ...
        responses: { '201': { description: 'Tag adicionada' } }
      },
      delete: {
        tags: ['Tags'],
        ...
        responses: { '200': { description: 'Tag removida' } }
      }
    },
    '/tags/tasks/{taskId}': {
      get: {
        ...
        security: [{ bearerAuth: [] }],
        parameters: [{ name: 'taskId', in: 'path', required: true, schema: { type: 'integer' } }],
        responses: { '200': { description: 'Lista de tags da tarefa' } }
      }
    }
  }
};
